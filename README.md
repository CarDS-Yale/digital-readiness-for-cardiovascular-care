...
choices. The first set vary how readiness is measured: rebuilding the index from raw ACS
and FCC inputs, splitting it into its infrastructure and socioeconomic halves, and
swapping the 2022 vintage for 2024. Additional analyses vary the workforce definition: primary care
supply instead of cardiology, a 25-mile cross-county access rule, and a pool that counts
cardiology NPs and PAs as coverage. The remaining relax the sample definitions and the cutoffs, including
a run with no workforce screen at all. Each run reports box
